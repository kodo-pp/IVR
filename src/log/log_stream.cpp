...
LogStream::LogStream(const std::function<std::wstring()>& _lineBeginnerFunc)
    ...
    std::wostream* logFile =
            new std::wofstream(logFileName, std::ios::out); // COMBAK: добавить MemoryManager
...
std::function<std::wstring()> LogStream::getLineBeginnerFunc() { return lineBeginnerFunc; }
...
void LogStream::beginLine(bool v = true) { doBeginLine = v; }

bool LogStream::getDoBeginLine() { return doBeginLine; }
